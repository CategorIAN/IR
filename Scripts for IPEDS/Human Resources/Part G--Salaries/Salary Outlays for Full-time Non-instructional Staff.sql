...
(SELECT  PPWG_HRP_ID,
        ...
        CASE
            WHEN (
                POS_SOC_CODE LIKE '25-4%'
                    OR POS_SOC_CODE LIKE '25-2%'
                    OR POS_SOC_CODE LIKE '25-3%'
                    OR POS_SOC_CODE LIKE '25-9%'
                ) THEN 'Library and Student and Academic Affairs and Other Education Services Occupations'
...
LEFT JOIN (
    SELECT PPWG_HRP_ID
    FROM PERPOSWG
    WHERE PPWG_PAYCLASS_ID IS NOT NULL
AND (
    PPWG_END_DATE IS NULL
    OR PPWG_END_DATE > GETDATE()
    )) AS X ON PERSON.ID = X.PPWG_HRP_ID
WHERE PERSTAT_END_DATE IS NULL
AND PERSTAT_START_DATE <= '2024-11-01'
AND PERSTAT_STATUS = 'FT'
AND POSITION.POS_CLASS != 'FAC'
...
SELECT * FROM PERPOSWG WHERE PPWG_HRP_ID = '5450448' ORDER BY PPWG_END_DATE DESC