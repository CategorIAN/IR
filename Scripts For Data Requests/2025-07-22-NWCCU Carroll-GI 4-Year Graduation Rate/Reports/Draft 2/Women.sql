...
SELECT School,
       Year,
       CAST(AVG(1.0 * FOUR_YEAR_GRADUATED) * 100 AS INT) AS [Graduation Rate (4 Years, GI Benefits, Women)]
FROM (
--(Begin 1)------------------------------------------------------------------------------------------------------------
         SELECT School,
                Year,
         ...
         FROM (VALUES ('Carroll College')) AS SCHOOLS(School)
             ...
             AND AWARD_TERMS.TERM_START_DATE < DATEADD(YEAR, 4, START_TERM.TERM_START_DATE)
             AND SA_ACTION = 'A'
