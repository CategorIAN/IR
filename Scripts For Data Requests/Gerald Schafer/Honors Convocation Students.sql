...
SELECT          STUDENT_ID,
                ...
                GPA_FOR_ACAD_LEVEL,
                CASE WHEN MAX(CASE
                    WHEN MAIN_MAJOR = 'PUBH' OR (ADDNL_MAJOR = 'PUBH' AND MAJOR_END_DATE IS NULL) THEN 1 ELSE 0 END) = 1
                    THEN 'Y' END AS PH,
                CASE WHEN MAX(CASE
                    WHEN MAIN_MAJOR = 'HSCI' OR (ADDNL_MAJOR = 'HSCI' AND MAJOR_END_DATE IS NULL) THEN 1 ELSE 0 END) = 1
                    THEN 'Y' END AS HS,
                MAJOR1
FROM (
--(Begin 1)----------------------------------------------------------------------------------------------
...
SELECT X.*,
       ...
       CAST(ENROLL_GPA_CREDITS AS INT) AS HONORS_CONVOCATION_MASTER_LIST_CREDITS,
       ENROLL_VERIFIED_GRADE AS HONORS_CONVOCATION_MASTER_LIST_GRADE_VERIFIED,
       CAST(ENROLL_GRADE_POINTS AS INT) AS TOTAL_GRADE_POINTS_FOR_COURSE
FROM (
--(Begin 2)-------------------------------------------------------------------------------------------------------------
         SELECT STUDENT_ID,
                NAME_LAST,
                ...
                CASE
                    WHEN MAX(CASE
                                 WHEN MAIN_MAJOR = 'PUBH' OR (ADDNL_MAJOR = 'PUBH' AND MAJOR_END_DATE IS NULL) THEN 1
                                 ELSE 0 END) = 1
                        THEN 'Y' END AS PH,
                CASE
                    WHEN MAX(CASE
                                 WHEN MAIN_MAJOR = 'HSCI' OR (ADDNL_MAJOR = 'HSCI' AND MAJOR_END_DATE IS NULL) THEN 1
                                 ELSE 0 END) = 1
                        THEN 'Y' END AS HS,
                MAJOR1
         FROM (
