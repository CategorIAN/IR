...
SELECT MAJOR,
       ...
       FORMAT(TWO_YEAR_GRADUATION_RATE, 'P') AS TWO_YEAR_GRADUATION_RATE,
...
--(Begin C3)------------------------------------------------------------------------------------------------------------
         SELECT MAJOR,
                ...
                CASE WHEN DATEDIFF(DAY, START, '2025-01-01') / 365.25 >= 2 THEN AVG(1.0 * TWO_YEAR_GRADUATE)
                    END AS TWO_YEAR_GRADUATION_RATE,
                CASE WHEN DATEDIFF(DAY, START, '2025-01-01') / 365.25>= 4 THEN AVG(1.0 * FOUR_YEAR_GRADUATE)
                    END AS FOUR_YEAR_GRADUATION_RATE,
               CASE WHEN DATEDIFF(DAY, START, '2025-01-01') / 365.25 >= 6 THEN AVG(1.0 * SIX_YEAR_GRADUATE)
                    END AS SIX_YEAR_GRADUATION_RATE,
...
--(Begin C2)------------------------------------------------------------------------------------------------------------
                  SELECT MAJOR,
                         ...
                         MAJOR_COMPLETED,
                         CASE
                             WHEN MAJOR_COMPLETED = 1 AND ACAD_END_DATE < DATEADD(YEAR, 2, START) THEN 1
                             ELSE 0 END AS TWO_YEAR_GRADUATE,
                         CASE
